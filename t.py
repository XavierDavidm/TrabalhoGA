...
class Pousada:
    def __init__(self): #atributos-> str(nome),str(contato),quartos-lista,reservas-lista,produtos-lista
        ...
        self.carregaDados() #função integrada de deserialização
    ...
    def carregaDados(self):
        ...
        if self.verificarArquivo('pousada.txt'):
        ...
        # Carrega os quartos
        if self.verificarArquivo('quartos.txt'):
            with open('quartos.txt', 'r') as ARQquartos:
                self.quartos = []
                for linha in ARQquartos:
                    dados = linha.strip().split(',')
                    quarto = Quarto(dados[0], dados[1], dados[2], dados[3])
                    self.quartos.append(quarto)

        # Carrega as reservas
        if self.verificarArquivo('reservas.txt'):
            with open('reservas.txt', 'r') as ARQreservas:
                self.reservas = []
                for linha in ARQreservas:
                    dados = linha.strip().split(',')
                    reserva = Reserva(dados[0], dados[1], dados[2], dados[3], dados[4])
                    self.reservas.append(reserva)

        # Carrega os produtos
        if self.verificarArquivo('produtos.txt'):
